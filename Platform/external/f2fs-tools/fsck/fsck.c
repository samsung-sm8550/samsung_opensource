						...
						struct node_info *ni)
{
	...
	if (!f2fs_is_valid_blkaddr(sbi, ni->blk_addr, DATA_GENERIC)) {
		ASSERT_MSG("blkaddress is not valid. ni->ino [0x%x], ni->nid [0x%x], blkaddr [0x%x]", ni->ino, ni->nid, ni->blk_addr);
		return -EINVAL;
	}
			...
			struct node_info *ni)
{
	...
	inode = &(node_blk->i);
	if ((c.feature & F2FS_FEATURE_COMPRESSION) && ntype == TYPE_INODE &&
		inode->i_flags & cpu_to_le32(F2FS_COMPR_FL) &&
		F2FS_FITS_IN_INODE(inode, le16_to_cpu(inode->i_extra_isize),
		i_log_cluster_size)) {
		if ((inode->i_compress_algorithm >= MAX_COMPRESS_ALGS) ||
			(inode->i_log_cluster_size < MIN_COMPRESS_LOG_SIZE) ||
			(inode->i_log_cluster_size > MAX_COMPRESS_LOG_SIZE)) {
	...
	if (f2fs_test_main_bitmap(sbi, ni->blk_addr) == 0) {
		...
		if (!c.show_file_map && sbi->total_valid_node_count > 1000) {
			...
			MSG(0, "[FSCK] Check node %"PRIu64" / %u (%.2f%%)\n",
...
static void print_disk_space_usage(struct f2fs_sb_info *sbi, u32 nid, u8 type)
{
	struct node_info ni;
	struct f2fs_node *node_blk;
	int inline_data;
	...
	if (!f2fs_is_valid_blkaddr(sbi, ni.blk_addr, DATA_GENERIC) ||
							ni.blk_addr == 0) {
		fprintf(stderr, "Invalid node blkaddr: %u ", ni.blk_addr);
		goto out;
	}
	...
	if (le32_to_cpu(F2FS_NODE_FOOTER(node_blk)->ino) != ni.ino ||
			le32_to_cpu(F2FS_NODE_FOOTER(node_blk)->nid) != ni.nid) {
		fprintf(stderr, "Invalid inode block");
		goto out;
	}

	inline_data = node_blk->i.i_inline & F2FS_INLINE_DATA;

	du = (le64_to_cpu(node_blk->i.i_blocks) - 1) * F2FS_BLKSIZE;
	if ((type == F2FS_FT_REG_FILE) && inline_data)
		du += round_up(node_blk->i.i_size, 1024);
	printf("%12llu %12llu ",
		(unsigned long long)le64_to_cpu(node_blk->i.i_size),
		...
		int max, int idx, int last_blk, int enc_name)
{
	...
	if (!c.show_dentry && !c.show_file_map && !c.show_disk_usage)
		return;
	...
	if (c.show_file_map) {
		...
		if (dump_node(sbi, le32_to_cpu(dentry[idx].ino), 0, NULL, 0, 0))
			printf("\33[2K\r");
	} else if (c.show_disk_usage) {
		struct f2fs_dentry *d = fsck->root_dentry;

		if (dentry[idx].file_type == F2FS_FT_DIR)
			return;

		print_disk_space_usage(sbi, le32_to_cpu(dentry[idx].ino),
						dentry[idx].file_type);
		while (d) {
			if (d->depth > 1)
				printf("/%s", d->name);
			d = d->next;
...
static int quota_inode_init_new(struct node_info ni, int qtype)
{
	int ret;
	struct f2fs_node *node_blk = NULL;
	...
	F2FS_NODE_FOOTER(node_blk)->nid = cpu_to_le32(ni.ino);
	F2FS_NODE_FOOTER(node_blk)->ino = cpu_to_le32(ni.ino);
	F2FS_NODE_FOOTER(node_blk)->cp_ver = cpu_to_le64(0xbab0);
	F2FS_NODE_FOOTER(node_blk)->next_blkaddr = cpu_to_le32(0xcafe);
	...
	node_blk->i.i_flags = cpu_to_le32(F2FS_IMMUTABLE_FL);
...
int fsck_chk_quota_node(struct f2fs_sb_info *sbi)
{
	...
	for (qtype = 0; qtype < F2FS_MAX_QUOTAS; qtype++) {
		...
		if (ret) {
			...
			cbc.cnt = 0;
			cbc.cheader_pgofs = CHEADER_PGOFS_NONE;
			quota_inode_init_new(ni, qtype);

			err = fsck_chk_node_blk(sbi, NULL, ni.ino,
					F2FS_FT_REG_FILE, TYPE_INODE, &blk_cnt, &cbc, NULL);
			if (err) {
...
void fsck_read_extra_flag_blk(struct f2fs_sb_info *sbi)
{
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	block_t extra_blk_no = get_sb(cp_blkaddr) - 1;
	int res;
...
int fsck_chk_need_fsck(struct f2fs_sb_info *sbi, bool quiet)
{
	struct f2fs_fsck *fsck = F2FS_FSCK(sbi);
	...
	if (le32_to_cpu(extra_blk->spo_counter) >= FSCK_SPO_CNT_THR)
		ret = 1;
...
void fsck_set_extra_fs_info(struct f2fs_sb_info *sbi)
{
	...
	extra_blk->valid_node_count = cp->valid_node_count;
	extra_blk->valid_inode_count = cp->valid_inode_count;
}
...
static u64 get_max_image_blks(struct f2fs_sb_info *sbi)
{
	struct f2fs_super_block *sb = F2FS_RAW_SUPER(sbi);
	struct f2fs_checkpoint *cp = F2FS_CKPT(sbi);
	u32 valid_main_segs;
	...
	int sb_seg = 1;
	int cp_segs = 2;
	int sit_segs, nat_segs, ssa_segs;
	...
	sit_segs *= 2;
	nat_segs = SEG_ALIGN(SIZE_ALIGN(get_cp(valid_node_count),
						NAT_ENTRY_PER_BLOCK));
	nat_segs *= 2;
	if (get_sb(feature) & F2FS_FEATURE_RO)
		ssa_segs = 0;
	else
		ssa_segs = SEG_ALIGN(valid_main_segs / sbi->blocks_per_seg + 1);
	max_image_segs = sb_seg + cp_segs + sit_segs + nat_segs +
					ssa_segs + valid_main_segs;

	if (max_image_segs < (F2FS_MIN_SEGMENTS + 1))
		return (u64)(F2FS_MIN_SEGMENTS + 1) * sbi->blocks_per_seg;
	else
		return max_image_segs * sbi->blocks_per_seg;
}
...
int fsck_verify(struct f2fs_sb_info *sbi)
{
	...
	if (c.show_file_map || c.show_disk_usage)
		return 0;
	...
	free_blks = (sbi->total_sections - data_secs - node_secs) *
							BLKS_PER_SEC(sbi);
	max_blks = get_max_image_blks(sbi);
	...
	if (force || (c.fix_on && f2fs_dev_is_writable())) {
		...
		if (force || c.bug_on || c.bug_nat_bits || c.quota_fixed) {
			...
			stlog("Info: FILE SYSTEM WAS MODIFIED (checked by fsck flag)\n");
		} else {
			char *msg;

			if (fsck_chk_need_fsck(sbi, true))
				msg = "extra flag or SPO threshold";
			else if (is_set_ckpt_flags(cp, CP_DISABLED_FLAG))
				msg = "cp disabled";
			else if (c.abnormal_stop)
				msg = "abnormal stop";
			else if (c.fs_errors)
				msg = "fs errors";
			else if (exist_qf_ino(sb) &&
					is_set_ckpt_flags(cp, CP_ERROR_FLAG))
				msg = "cp error";
			else
				msg = "unknown";
			stlog("Info: FILE SYSTEM WAS MODIFIED (checked by %s)\n", msg);
		}
	...
	stlog("Info: %s %llu blocks, %u files, %u/%u nodes, %llu/%llu blocks, %u SPO cnt\n",
			c.devices[0].path,
			(unsigned long long)get_sb(block_count),
			fsck->chk.valid_inode_cnt,
			fsck->chk.valid_node_cnt, sbi->total_node_count,
